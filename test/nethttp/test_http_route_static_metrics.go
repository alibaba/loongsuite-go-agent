...
func setupStaticPatternMetricHttp() {
...
func main() {
	go setupStaticPatternMetricHttp()
	...
	verifier.WaitAndAssertMetrics(map[string]func(metricdata.ResourceMetrics){
		"http.server.request.duration": func(mrs metricdata.ResourceMetrics) {
			...
			verifier.VerifyHttpServerMetricsAttributes(point.DataPoints[0].Attributes.ToSlice(), "GET", "/query", "", "http", "1.1", "http", 200)
		},
	})
}
